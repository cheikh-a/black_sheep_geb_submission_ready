...
Fix a balanced theft cycle $\sigma$ and suppose every agent enters the period with wealth $\bar{w}$. By definition of a balanced cycle, each agent $i$ undertakes exactly one successful outgoing raid against $\sigma(i)$ and suffers exactly one incoming successful raid by $\sigma^{-1}(i)$. Equation \eqref{eq:benchmark-wealth-update} therefore becomes
\[
...
Take an arbitrary agent $i$ in a balanced theft cycle. If $i$ follows the prescribed action, he pays the effort cost $c_T$, loses one unit to the incoming raid, and gains one unit from his outgoing raid, finishing with $\bar{w} - c_T$. Consider three classes of unilateral deviations.

First, suppose $i$ deviates by raiding a different victim while still going out to steal. Since all nondeviating households remain absent under the balanced-cycle profile, the alternative victim is still vulnerable. The deviator still obtains one unit of loot and still suffers the incoming raid from $\sigma^{-1}(i)$, so the deviating payoff remains $\bar{w} - c_T$. Such a deviation is therefore payoff equivalent.

Second, suppose $i$ deviates by staying home. Then the raid on $i$ fails because the house is occupied, but $i$ gives up the one unit he would otherwise have taken from $\sigma(i)$. The deviating payoff is $\bar{w} - \tau$. Since $c_T \le \tau$, following the cycle yields $\bar{w} - c_T \ge \bar{w} - \tau$, so staying home is not strictly profitable.

Third, suppose $i$ contemplates a mixed deviation over alternative victims and staying home. Since neither pure deviation is strictly profitable, no mixture of them is strictly profitable either. Hence no unilateral deviation improves on the balanced-cycle payoff, and the profile is a weak Nash equilibrium. \qed
...
This subsection records the simulation algorithm in enough detail that a reader can map every figure in the paper to a precise computational step. The code itself is the authoritative implementation, but the written algorithm helps align the appendix with the manuscript's notation.
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Compute candidate personal theft returns for rich agents using \eqref{eq:personal-payoff}. Compute candidate contract surplus using \eqref{eq:total-surplus} and accept only contracts satisfying \eqref{eq:feasibility}. Order patrons from richest to poorest and match them to the poorest available workers.
    ...
    \item Realize raids sequentially. Each realized transfer is based on the victim's remaining wealth at the moment of attack, so multiple attacks on the same house have diminishing returns within a period.
...
\begin{table}[tbp]
    ...
    \input{tables/table_regime_frequencies.tex}
\end{table}
