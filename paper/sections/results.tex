...
Figure \ref{fig:fig02_first_night_accounting} compresses the same event into a cleaner accounting display. The honest agent is protected on the first night, the successor receives the first windfall, and the predecessor is the first loser. Theorem \ref{thm:no-stationary-one-honest} uses exactly that chronology through the local state change that places the successor at $\bar{w}+\tau$ and compares that wealth to the cutoff in \eqref{eq:threshold}. A different first-night chronology would imply a different state and a different continuation result.
...
The deterministic benchmark leaves no room to attribute the departure from the original cycle to chance. The simulation is implementing the same occupancy rule, the same first-night accounting, and the same cutoff comparison used in the theorem. Once successor wealth exceeds \eqref{eq:threshold}, staying home dominates further personal raiding, and the event-time display records the realized implication of that threshold comparison.

\subsection{From local asymmetry to class formation}

Longer horizon simulations show how the local state change created by Lemma \ref{lem:first-night} becomes a persistent class structure once agents are sorted by the payoff comparisons in Section \ref{sec:model}. Figure \ref{fig:fig03_event_time_roles} groups agents by the highest institutional role they reach during the transition and traces the corresponding wealth paths in event time, a grouping that preserves the early separation because many agents later withdraw into staying or inactivity. Agents who later become patrons separate first, agents who later serve as guards remain below them, and the honest household falls away sharply. This is the simulated counterpart of the exposure logic in \eqref{eq:self-theft-threshold}: early windfall recipients stop exposing wealth sooner than the rest of the population, and later organizational roles inherit that initial gap rather than creating it from nothing.
...
Taken together, Figures \ref{fig:fig03_event_time_roles} through \ref{fig:fig05_inequality_dynamics} show the same mechanism at three levels. The deterministic shock creates a local state difference, the cutoff in \eqref{eq:threshold} turns that difference into differentiated behavior, and the proportional environment amplifies the split because wealthy households cease to expose their own stocks while poor households remain available for predatory labor. The figures supply timing and magnitude; the underlying margins are already in the model.

\subsection{Patronage and the surplus from delegation}

Across an interpretable range of parameters, rich households hire poor thieves only when the contract yields the surplus described by \eqref{eq:total-surplus} and Proposition \ref{prop:patronage}. Figure \ref{fig:fig06_contracting_comparative_statics} examines that condition directly. The left panel varies the targeting advantage of delegation. The right panel traces compensation and the outside option as worker slackness changes. Patron surplus rises with the first margin and falls as the second margin strengthens, exactly as \eqref{eq:patron-payoff} and Corollary \ref{cor:patronage-statics} imply. Delegated theft that is better targeted than freelancing leaves room to satisfy participation and still retain a positive residual. A stronger outside option narrows that residual and can eliminate it.
...
Figure \ref{fig:fig08_role_shares} restores time to the same regime logic. The share of agents engaged in direct or freelance theft falls after the first perturbation. Patrons and hired thieves rise next. Guards appear later and remain active through much of the enforcement phase, even though their share is not monotone over time. The plot is useful precisely because each movement corresponds to a formal object already in the theory. The early decline in universal theft reflects Theorem \ref{thm:no-stationary-one-honest}; the rise in patrons and hired thieves reflects Proposition \ref{prop:patronage}; the later appearance of guarding reflects Theorem \ref{thm:guarding}.
...
Coexistence reveals institutional layering instead of a synchronized regime switch. In those paths, some elites have crossed the guard threshold while others still rely on patronage, so different technologies of control operate at the same time in different parts of the wealth distribution. The move from patronage to guarding is therefore unequal and sequential because \eqref{eq:guard-threshold} is wealth-dependent.

Table \ref{tab:simulation-summary} compresses the same sequence across the baseline perturbation, patronage, and enforcement simulations after the timing, regime, and occupational evidence is in place. The baseline perturbation is the least unequal of the three simulated environments. Patronage produces a larger rise in concentration because it adds a contract technology that channels loot upward. Enforcement then preserves and slightly intensifies that concentration. The table is therefore the compact numerical counterpart to the welfare logic in Proposition \ref{prop:welfare}: distributional divergence begins when reciprocity breaks and later regimes magnify or lock in that divergence through contracting and protection.
...
Figure \ref{fig:fig12_robustness_panels} collects the main one-dimensional robustness exercises. Final welfare responds most strongly to theft intensity and occupied-house vulnerability in this sweep, while final inequality moves within a narrower band. Regime classification also changes little once the system is already in the region where guarding is attractive. Among the four margins shown, only the theft-intensity sweep changes the dominant regime, passing from transition to guard and then to coexistence as extraction becomes harsher. Figure \ref{fig:fig12_robustness_panels} is therefore mainly about the strength of the mechanism, while Figure \ref{fig:fig07_regime_phase_diagram} remains the main source for regime geometry.
